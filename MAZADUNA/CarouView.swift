...
public class CarouView: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    public init(frame: CGRect, imageSet:[String], rideDirection:CarouDirection = .rightToLeft) {
    ...
    func setupPager() {
        ...
        guard self.images.count > 1 else {
            ...
            imageView.kf.setImage(with: URL(string: self.images[0]))//UIImageView(image: self.images[0])
            self.addSubview(imageView)
            return
            
        }
        
        self.scrollDirection = .rightToLeft
        ...
        self.dotColor = .white
        self.dotSize = .small
        self.currentDotColor = #colorLiteral(red: 0.8212025762, green: 0.8766968846, blue: 0.8450177312, alpha: 1)
        if self.autoScrollEnabled{
            self.timer = Timer.scheduledTimer(timeInterval: self.timeInterval, target: self, selector: #selector(timedOut(sender:)), userInfo: nil, repeats: true)
        }
    }
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        imageView.kf.setImage(with: URL(string: self.images[indexPath.row]))
        imageView.contentMode = .scaleAspectFit
